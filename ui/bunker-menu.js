...
(() => {
  ...
  // Leonov has two exclusive workspaces. Move the original nodes; never clone IDs
  // or replace the server-authoritative breeding/buying/selling implementations.
  function createLeonovWorkspace() {
    ...
    if (!root || !breed || !buy || !sell || !first || !second) return null;
    const nativeOpen = window.openScreen, nativeRender = window.renderScientists;
    const nativeBreed = window.doBreedArtifacts, nativeExit = window.exitScientists;
    const nativeBuy = window.openScientistsBuyView, nativeSell = window.openScientistsSellView;
    let mode = 'hub', activeSlot = 1, busy = false, gesture = null, ghost = null;
    let dragFrame = 0, suppressUntil = 0;
    const selection = document.createElement('section');
    selection.id = 'leonovSelectionPanel'; selection.setAttribute('aria-label', 'Селекция артефактов');
    const trade = document.createElement('section');
    trade.id = 'leonovTradePanel'; trade.setAttribute('aria-label', 'Торговля у Леонова');
    const slots = first.parentElement, description = slots.previousElementSibling;
    root.insertBefore(selection, description);
    selection.append(description, slots, document.getElementById('breedResultInfo'), actions);
    const tradeActions = document.createElement('div');
    tradeActions.className = 'leonov-trade-actions'; tradeActions.append(buy, sell);
    trade.append(tradeActions, document.getElementById('scientistsBuyList'), document.getElementById('scientistsSellList'));
    selection.after(trade);
    breed.id = 'leonovBreedButton'; buy.id = 'leonovBuyButton'; sell.id = 'leonovSellButton';
    buy.textContent = 'Купить'; sell.textContent = 'Продать';
    const hint = document.createElement('p'); hint.id = 'leonovDragHint'; hint.setAttribute('role', 'status');
    const inventoryTitle = document.createElement('h4'); inventoryTitle.textContent = 'Инвентарь — артефакты';
    const inventory = document.createElement('div'); inventory.id = 'leonovArtifactInventory';
    ...
    backButtons.forEach((button, i) => { button.hidden = i > 0; button.textContent = 'Назад'; });
    const heading = root.querySelector('.zr-title');
    const hero = root.querySelector('.zr-hero'); if (hero) hero.hidden = true;
    [first, second].forEach((slot, i) => {
      slot.dataset.leonovDrop = String(i + 1); slot.tabIndex = 0; slot.setAttribute('role', 'button');
      slot.onkeydown = e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openBreedSlotPicker(i + 1); } };
    });
    const style = document.createElement('style'); style.id = 'leonovModesStyle';
    style.textContent = `
      ...
      #leonovArtifactInventory .leonov-artifact-cell {min-width:0;min-height:82px;aspect-ratio:auto;
        padding:4px!important;display:flex;flex-direction:column;gap:3px;touch-action:none;
        user-select:none;-webkit-user-select:none;-webkit-touch-callout:none;letter-spacing:0;text-transform:none}
      ...
      .leonov-drag-ghost {position:fixed;left:0;top:0;width:66px;min-height:70px;z-index:3000;
        pointer-events:none;background:#171810;border:1px solid #d3ba78;border-radius:5px;padding:5px;
        display:flex;flex-direction:column;align-items:center;font:10px/1.1 'Segoe UI',sans-serif;opacity:.94}
      .leonov-drag-ghost img {max-width:45px;max-height:40px;object-fit:contain}
    `;
    document.head.append(style);
    const say = message => { hint.textContent = message || 'Удерживай артефакт и перетащи в слот 1 или 2. Нажатие на заполненный слот — убрать.'; };
    const quantity = name => Math.max(0, Number(player.inventory?.[name]) || 0);
    const eligible = name => { const def = findArtifactDef(name); return quantity(name) > 0 && def && !def.isNamedArtifact; };
    ...
    function setBusy(value) {
      busy = value; selection.setAttribute('aria-busy', String(value));
      breed.disabled = value; breed.textContent = value ? 'Селекция…' : 'Селекционировать';
    ...
    function refreshInventory() {
      ...
      for (const name of Object.keys(player.inventory || {}).filter(eligible)) {
        const cell = document.createElement('button'); cell.type = 'button';
        cell.className = 'slot leonov-artifact-cell'; cell.dataset.leonovItem = name;
        ...
        cell.setAttribute('aria-label', name + ', в рюкзаке ' + quantity(name));
        cell.append(label, count); fragment.append(cell);
      }
      if (!fragment.childNodes.length) {
        const empty = document.createElement('p'); empty.className = 'leonov-empty';
        empty.textContent = 'В рюкзаке нет артефактов, доступных для селекции.'; fragment.append(empty);
      }
    ...
    function place(name, slot) {
      ...
      if (!canPlace(name, slot)) { say('Для двух одинаковых артефактов нужны два экземпляра в рюкзаке.'); return; }
      if (slot === 1) breedSlot1 = name; else breedSlot2 = name;
      activeSlot = slot === 1 ? 2 : 1; refreshInventory(); say('Артефакт помещён в слот ' + slot + '.');
    }
    function cleanup() {
      if (gesture) clearTimeout(gesture.timer);
      cancelAnimationFrame(dragFrame); ghost?.remove(); ghost = null; gesture = null;
    ...
    function paint() {
      ...
      if (!isSelection()) { cleanup(); return; }
      const g = gesture; ghost.style.transform = `translate(${g.x - 33}px,${g.y - 85}px)`;
    ...
    function beginDrag() {
      ...
      gesture.active = true;
      ghost = document.createElement('div'); ghost.className = 'leonov-drag-ghost';
      ghost.setAttribute('aria-hidden', 'true'); ghost.innerHTML = gesture.source.innerHTML;
      document.body.append(ghost); paint();
    }
    inventory.addEventListener('pointerdown', e => {
      ...
      gesture = {id:e.pointerId,source,name:source.dataset.leonovItem,touch:e.pointerType === 'touch',
        x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY,lastY:e.clientY,active:false};
    ...
    document.addEventListener('pointermove', e => {
      ...
      if (!g.active && Math.hypot(g.x-g.startX,g.y-g.startY) > 8) {
        if (g.touch) { clearTimeout(g.timer); g.scrolling = true; } else beginDrag();
      }
      if (g.scrolling) { root.scrollTop += g.lastY - g.y; suppressUntil = Date.now() + 500; }
      g.lastY = g.y; if (g.active || g.scrolling) e.preventDefault();
    }, {passive:false});
    document.addEventListener('pointerup', e => {
      const g = gesture; if (!g || e.pointerId !== g.id) return;
      const target = g.active ? targetAt(e.clientX, e.clientY) : null; cleanup();
      if (g.active) {
        suppressUntil = Date.now() + 500;
        if (target) place(g.name, Number(target.dataset.leonovDrop)); else say('Перетаскивание отменено.');
      }
    });
    document.addEventListener('pointercancel', cleanup); window.addEventListener('blur', cleanup);
    ...
    document.addEventListener('click', e => {
      if (root.contains(e.target) && (busy || Date.now() < suppressUntil)) { e.preventDefault(); e.stopImmediatePropagation(); }
    }, true);
    inventory.addEventListener('click', e => {
      const item = e.target.closest('[data-leonov-item]'); if (!item) return;
    ...
    new MutationObserver(() => { if (!root.classList.contains('active')) cleanup(); })
      .observe(root, {attributes:true,attributeFilter:['class']});
    window.renderScientists = function() { if (mode === 'selection') refreshInventory(); else nativeRender(); };
    window.openBreedSlotPicker = function(slot) {
      if (!isSelection() || busy || (slot !== 1 && slot !== 2)) return;
      activeSlot = slot;
      if (slot === 1) breedSlot1 = null; else breedSlot2 = null;
      refreshInventory(); say('Перетащи артефакт в слот ' + slot + ' или нажми на него в инвентаре.');
    };
    window.doBreedArtifacts = async function() {
      if (!isSelection() || busy) return;
      cleanup(); setBusy(true);
      try { await nativeBreed(); }
      finally { setBusy(false); if (mode === 'selection') refreshInventory(); }
    };
    window.openScientistsBuyView = function() {
      if (mode !== 'trade') return;
      buy.setAttribute('aria-pressed', 'true'); sell.setAttribute('aria-pressed', 'false'); return nativeBuy();
    };
    window.openScientistsSellView = function() {
      if (mode !== 'trade') return;
      buy.setAttribute('aria-pressed', 'false'); sell.setAttribute('aria-pressed', 'true'); return nativeSell();
    };
    window.exitScientists = function() {
      if (busy) return;
      cleanup(); mode = 'hub'; nativeExit(); openLeonov();
    };
    // Warehouse/PDA entry points must also land on the hub, not the old mixed view.
    window.openScreen = function(screen) {
      if (screen === 'scientists') { if (busy) return; cleanup(); nativeOpen('main'); return openLeonov(); }
      cleanup(); return nativeOpen.apply(this, arguments);
    };
    selection.hidden = true; trade.hidden = true; say();
    return {
      open(nextMode) {
        if (busy || !['selection', 'trade'].includes(nextMode)) return;
        cleanup(); mode = nextMode; root.dataset.leonovMode = mode;
        selection.hidden = mode !== 'selection'; trade.hidden = mode !== 'trade';
        heading.textContent = mode === 'selection' ? 'Леонов — Селекция' : 'Леонов — Торговля';
        // Opening a view never activates a purchase, sale or breeding transaction.
        document.getElementById('scientistsBuyList').replaceChildren();
        document.getElementById('scientistsSellList').replaceChildren();
        buy.setAttribute('aria-pressed', 'false'); sell.setAttribute('aria-pressed', 'false');
        activeSlot = !breedSlot1 ? 1 : !breedSlot2 ? 2 : 1;
        nativeOpen('scientists'); root.scrollTop = 0; say();
      }
    };
  }
