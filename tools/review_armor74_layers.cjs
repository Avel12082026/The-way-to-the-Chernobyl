...
const candidate={weapon:1,armor:74,fit:{x:-245,y:-385,width:1250,height:1250},frontContours:[[[415,387],[480,350],[570,337],[665,363],[735,401],[880,440],[1040,590],[1536,800],[1536,1024],[850,1024],[670,825],[665,715],[662,575],[650,470],[580,450],[545,465],[420,460]],[[420,470],[545,462],[605,482],[605,548],[577,564],[437,563]],[[439,565],[580,563],[617,588],[610,647],[580,667],[440,662]],[[453,668],[580,664],[616,687],[612,734],[581,750],[456,742]]],status:'candidate-smoothed-contours-proportional-weapon'};
...
ctx.save();ctx.beginPath();for(const poly of candidate.frontContours){const last=poly[poly.length-1],first=poly[0];ctx.moveTo((last[0]+first[0])/2,(last[1]+first[1])/2);poly.forEach(([x,y],i)=>{const next=poly[(i+1)%poly.length];ctx.quadraticCurveTo(x,y,(x+next[0])/2,(y+next[1])/2);});ctx.closePath();}ctx.clip();ctx.drawImage(hand,0,0);ctx.restore();
