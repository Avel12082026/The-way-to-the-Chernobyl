...
(async()=>{
const weapon=Number(process.argv[2]),armor=Number(process.argv[3]),version=process.argv[4]||'fire-v1';
if(!Number.isInteger(weapon)||weapon<1||!Number.isInteger(armor)||armor<1||armor>96)throw Error('Invalid pair');
if(!/^fire-v[0-9]+$/.test(version))throw Error('Invalid version');
const dir=`asset_sources/forward_grips/${weapon}-${armor}/${version}-stable-review`;
fs.mkdirSync(dir,{recursive:true});
const [fg,bg,enemy]=await Promise.all([loadImage(`asset_sources/forward_grips/${weapon}-${armor}/${version}.webp`),loadImage('images/combat/backgrounds/zombie/1.png'),loadImage('images/combat/mutants/zombie.png')]);
const fit=JSON.parse(fs.readFileSync(`asset_sources/forward_grips/${weapon}-${armor}/${version}-placement.json`));
...
for(const [i,ms] of [-1,0,40,110,150,240].entries()){
 ...
 paired.draw(ctx,fg,{...fit,weaponId:weapon,suppressed:false},{shot:ms>=0,elapsed:ms},effects);
