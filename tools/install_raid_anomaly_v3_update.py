#!/usr/bin/env python3
"""Install the raid anomaly V3 update with strict source checks and code-only rollback."""
from pathlib import Path
import argparse, fcntl, hashlib, json, os, shutil, subprocess, sys, tempfile, time, urllib.request

from patch_raid_survival import build, MARK
...
def atomic(path,content,st=None):
    fd,name=tempfile.mkstemp(prefix='.raid-anomaly-v3-',suffix=path.suffix,dir=path.parent)
    try:
...
def prepare(root,payload):
    ...
    if not server_path.exists(): raise RuntimeError('server.js не найден')
    before=server_path.read_bytes()
    source=before.decode('utf-8')
    module=payload.read_bytes()
    if digest(module)!=MODULE_HASH:
        raise RuntimeError('Не совпал SHA-256 raid-survival.cjs')

    # A repeat run is allowed only when the exact new module is already present and
    ...
    if MARK in source:
        ...
        if server_hash not in KNOWN_SERVER_INPUTS:
            raise RuntimeError('server.js отличается от проверенных версий. Ничего не установлено. SHA='+server_hash)
        patched=build(source).encode('utf-8')
        if MARK.encode() not in patched:
            raise RuntimeError('Новый маркер рейдового патча не появился')
        new={'server.js':patched,'raid-survival.cjs':module}

    with tempfile.TemporaryDirectory(prefix='raid-anomaly-v3-check-') as td:
        for name,data in new.items():
            candidate=Path(td)/name
            candidate.write_bytes(data)
            run(['node','--check',str(candidate)])
    return {'server.js':before},new
...
def healthy():
    try:
        ...
        return raid.get('success') is True and raid.get('version')==VERSION and raid.get('travelCost')==2 and isinstance(market,list)
    except Exception:
        return False
...
def deploy(root,before,updates,control=service,probe=healthy,wait_seconds=40):
    ...
    backup=root/('BACKUP_BEFORE_RAID_ANOMALY_V3_'+stamp)
    ...
    try:
        ...
        for name,data in updates.items():
            atomic(root/name,data,stats[name])
        control('start')
        deadline=time.monotonic()+wait_seconds
        while time.monotonic()<deadline:
            if probe(): return backup
            time.sleep(1)
        raise RuntimeError('API не подтвердил версию '+VERSION+' за время ожидания')
