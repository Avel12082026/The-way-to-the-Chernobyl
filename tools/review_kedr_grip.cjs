...
const dir=`asset_sources/forward_grips/8-91/${version}-stable-review`;
fs.mkdirSync(dir,{recursive:true});
const [fg,bg,enemy]=await Promise.all([loadImage(`asset_sources/forward_grips/8-91/${version}.webp`),loadImage('images/combat/backgrounds/zombie/1.png'),loadImage('images/combat/mutants/zombie.png')]);
const fit=JSON.parse(fs.readFileSync(`asset_sources/forward_grips/8-91/${version}-placement.json`));
...
for(const [i,ms] of [-1,0,40,110,150,240].entries()){
 ...
 paired.draw(ctx,fg,{...fit,weaponId:8,suppressed:false},{shot:ms>=0,elapsed:ms},effects);
