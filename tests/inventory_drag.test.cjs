...
class El{
 ...
 addEventListener(n,cb){this.events[n]=cb} querySelectorAll(){return targets} closest(s){return s.includes('data-drag-item')?(this.dataset.dragItem?this:null):this.dataset.dropKind?this:null}
}
const ids={};for(const id of ['inventoryScreen','inventoryGrid','quickSlotsGrid','artifactSlotsGrid','inventoryDragHint'])ids[id]=new El();
ids.inventoryScreen.classes.add('active');ids.inventoryGrid.children=Array.from({length:6},()=>new El());ids.quickSlotsGrid.children=Array.from({length:7},()=>new El());ids.artifactSlotsGrid.children=Array.from({length:6},()=>new El());
...
const player={inventory:{gun:1,armor:1,detector:1,artifact:1,medkit:3,loot:1},artifactSlots:[null,null,null,null,null,null],quickSlots:[null,null,null,null]};
...
const down=(i,touch=false)=>ids.inventoryScreen.events.pointerdown({target:ids.inventoryGrid.children[i],pointerId:1,button:0,isPrimary:true,pointerType:touch?'touch':'mouse',clientX:100,clientY:500});
...
(async()=>{
 ...
 let blocked=false;ids.inventoryScreen.events.click({preventDefault(){blocked=true},stopImmediatePropagation(){}});assert(blocked,'drag must not open click menu');
