...
for(const [name,fit] of Object.entries(layout.placement)){
 const hand=await loadImage(process.env.HAND_IMAGE||('images/anomaly/hands/'+[1,16,70][i%3]+'_right.webp')),detector=await loadImage('images/anomaly/items/'+name.replace('.jpg','.webp'));
...
}fs.writeFileSync(process.env.REVIEW_OUTPUT||'.validation/grip-review.jpg',sheet.toBuffer('image/jpeg'));console.log('Rendered nine model placements and verified three antenna mask dimensions; visual review required');})().catch(e=>{console.error(e);process.exit(1)});
