...
for(let armorId=1;armorId<=96;armorId++){
 ...
 for(const weaponId of Object.keys(f.data.weapons))if(Number(weaponId)!==43)assert.equal(f.resolve({armorId,weaponId}).adjustment?.supportShift,undefined,'Compact path must not alter another weapon');
}
