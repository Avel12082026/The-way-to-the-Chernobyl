...
test('Kedr uses its own sprite and profile',async()=>{const h=harness();assert.equal(await h.window.CombatScene.show({...next,weaponId:8}),true);assert(h.urls.some(x=>x.startsWith('images/combat/paired/8-91.webp')));assert.equal(h.draws[0][2].weaponId,8);assert.equal(h.draws[0][2].muzzle[0],690);});
