...
function draw(now=0){
 ...
 ctx.save();
 if(active&&reaction.shot)ctx.translate(0,Math.sin(Math.PI*Math.min(1,elapsed/220))*12);
 if(pictures[2]&&pictures[3]){
  const drawn=root.CombatLayout.drawForeground(ctx,pictures[2],pictures[3],config.weaponId);
  // Barrel and effect share the recoil transform; missing foreground cannot fire.
  if(drawn&&!reduced.matches)root.CombatEffects?.drawShot(ctx,reaction,elapsed,config.weaponId,root.COMBAT_ASSETS.pistols.find(w=>w.id===config.weaponId));
 }
 ctx.restore();
